...
async function loginPost(req, res) {
  try {
    ...
    const token = jwt.sign(user.dataValues, config.auth.jwtKey, {
...
async function activate(req, res) {
  try {
    ...
    const localUser: any = await userRepository.getUserByActivationToken(token);
...
async function resetPassword(req, res) {
  try {
    ...
    const localUser: any = await getUserByResetToken(token);
...
async function resetPasswordPost(req, res) {
  try {
    ...
    const localUser: any = await getUserByResetToken(data.token);
...
async function getUserByResetToken(token) {
  ...
  const localUser: any = await userRepository.getUserByResetToken(token);
